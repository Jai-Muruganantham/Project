# -*- coding: utf-8 -*-
"""
Created on Fri Mar  1 14:15:24 2019
...
def get_frame():
    url = 'http://10.20.230.36:8085/shot.jpg'  # Replace 'your_ip_address' with the actual IP address of your mobile device
...
def AdminUser():
    ...
    MianAttendanceWindow = tk.Button(windowAdminUser, text="Start Attendance", command=StartAttendance  ,fg="red"  ,bg="sky blue"  ,width=20  ,height=1, activebackground = "Red" ,font=('times', 15, ' bold '))
    MianAttendanceWindow.place(x=650, y=450)
    quiteWindow = tk.Button(windowAdminUser, text="LongOut", command=AdminLogin1  ,fg="red"  ,bg="sky blue"  ,width=20  ,height=1, activebackground = "Red" ,font=('times', 15, ' bold '))
    ...
    while True:
        ...
        for(x,y,w,h) in faces:
            ...
            if(conf < 50):
                ...
                aa=df.loc[df['Id'] == Id]['Name'].values
                tt=str(Id)+"-"+aa
                k="jaimruganantham@gmail.com"
                attendance.loc[5] = [Id,aa,date,timeStamp,k]    
            else:
                Id='Unknown'                
                tt=str(Id)  
            if(conf > 75):
