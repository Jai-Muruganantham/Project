...
url='http://10.21.54.248:8080//shot.jpg'
def AdminLogin():
    windowAdmin=tk.Tk()
...
def AdminUser():
    ...
    quiteWindow = tk.Button(windowAdminUser, text="Log out", command=AdminLogin1  ,fg="red"  ,bg="sky blue"  ,width=20  ,height=1, activebackground = "Red" ,font=('times', 15, ' bold '))
...
def StartAttendance(Email):
    ...
    c_emails=['Email']
    attendance = pd.DataFrame(columns = col_names)  
    def Email(email,fileName,msm,n):
        ...
        if(msm=='a'):
            ...
            body = "Government college of engineering Computer science and engineering department Hall Ticket Generated Your Attendance Percentage is "+str(n)+"%"
    ...
    cv2.destroyAllWindows()
    conn=sqlite3.connect("StudentDataBase.db") 
    Atten=conn.execute("select Period from AttendancePeriod")
    def check(): 
        cmd="SELECT Period from AttendancePeriod"
        cursur1=conn.execute(cmd) 
        isRecordExit=0
        for row in cursur1: 
            isRecordExit=1 
    ...
    if(record):
        ...
        for period in ps:
            s=period[0]
            s=s+1
            conn.execute("Update AttendancePeriod set Period=?",(s,))
            print("Updated Attendance")
    else:
            conn.execute("insert into AttendancePeriod values(1)")
            print("Inserted Attendance")
    conn.commit()
    conn.close()
    Email(email,fileName,'a',2)
                    ...
                    for row2 in csv_reader2:
                        if line_count == 0:
                            ...
                            if(s==row2[0]):
                                email=f'{row2[2]}'
                                def check(UserId): 
                                    cmd="SELECT * FROM StudentPeriod Where Id="+str(UserId)
                                    cursur1=conn.execute(cmd) 
                                    isRecordExit=0
                                    for row in cursur1: 
                                        isRecordExit=1 
                                    return isRecordExit
                                isRecordExit=check(int(row2[0]))
                                if(isRecordExit):
                                    #Alter
                                    cmds="Select Period from StudentPeriod Where Id="+str(row2[0])
                                    PeriodNo=conn.execute(cmds)
                                    for p in PeriodNo:
                                        ext=int(p[0])
                                        ext=ext+1
                                        conn.execute("Update StudentPeriod set Period=? where Id=? ",(ext,row2[0]))
                                    print("Updated")
                                else:
                                    conn.execute("insert into StudentPeriod values(?,1) ",(row2[0],))
                                    print("Inserted "+str(row2[0]))
                                #per=float(tota)*(100/500)
                                hall="Select Period From StudentPeriod where Id="+str(row2[0])
                                atten="Select Period From AttendancePeriod"
                                ss=conn.execute(atten)
                                cs=conn.execute(hall)
                                per=0
                                for AP in ss:
                                    for SP in cs:
                                        per=float(SP[0])*(100/5)
                                        print(per)
                                        print(AP[0])
                                        print(SP[0])
                                    if((per)>=80 and (AP[0]==5)):
                                        print("HallTicketGenerated")
                                        file="Hall Ticket.docx"
                                        Email(email,file,'s',per)
                                        conn.execute("Update StudentPeriod set Period=? where Id=? ",(0,row2[0]))
                                        conn.execute("Update AttendancePeriod set Period=0")
                                    if(per<80 and AP[0]==5):
                                        print("Hall ticket not generated for the student"+str(row2[0]))
                                    if(AP[0]>=5):
                                       conn.execute("Update StudentPeriod set Period=? where Id=? ",(0,row2[0]))
                                       conn.execute("Update AttendancePeriod set Period=0")
